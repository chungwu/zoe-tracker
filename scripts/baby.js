...
(function() {
  var FEED_ACTIONS, containsAction, containsActions, customBoundaryCond, dateBoundaryCond, deserializeEventRows, deserializeKeyRows, generateStateEvents, generateSummaries, getEventsDateRange, groupEventsByBoundary, handleData, intervalOverlap, renderEvents, renderGroupHeader, renderSummaries, renderTimelines, rowsToEvents, setRange, setTimelineRanges, timeIn;
  ...
  renderGroupHeader = function(title, events, stateTrendKeys, eventTrendKeys) {
    var $aggs, $container, dateRange, event, eventStats, k, key, stateStats, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3, _ref4;
    $container = $("<div/>");
    ...
    for (_i = 0, _len = events.length; _i < _len; _i++) {
      ...
      if (event.stateKey != null) {
        ...
        for (_k = 0, _len2 = eventTrendKeys.length; _k < _len2; _k++) {
          ...
          if (containsActions([(_ref = event.eventKey) != null ? _ref.name : void 0, event.text], key.value)) {
    ...
    for (_l = 0, _len3 = stateTrendKeys.length; _l < _len3; _l++) {
      ...
      $("<li/>").append($("<span class='agg-key'/>").text("" + key.name).css("backgroundColor", (_ref2 = key.backgroundcolor) != null ? _ref2 : "").css("color", (_ref1 = key.foregroundcolor) != null ? _ref1 : "")).append($("<span class='agg-val'/>").text("" + (moment.duration(stateStats[key.name]).humanize()))).appendTo($aggs);
    }
    for (_m = 0, _len4 = eventTrendKeys.length; _m < _len4; _m++) {
      key = eventTrendKeys[_m];
      $("<li/>").append($("<span class='agg-key'/>").text("" + key.name).css("backgroundColor", (_ref4 = key.backgroundcolor) != null ? _ref4 : "").css("color", (_ref3 = key.foregroundcolor) != null ? _ref3 : "")).append($("<span class='agg-val'/>").text("" + eventStats[key.name] + "x")).appendTo($aggs);
    }
    return $container;
  };

  renderTimelines = function(allEvents, mode, alignKeys, eventTrendKeys, stateTrendKeys) {
    var $body, $summariesTimeline, $table, $timeline, $tr, boundaryCond, events, groupedEvents, handlingEvent, summariesTimeline, timeline, timelines, title, _i, _j, _len, _len1, _ref, _ref1;
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      _ref1 = _ref[_i], title = _ref1[0], events = _ref1[1];
      $tr = $("<tr/>").appendTo($table);
  ...
  setTimelineRanges = function(mode, timelines) {
    var dateRange, i, maxDuration, r, ranges, t, timeline, _i, _j, _len, _len1, _results, _results1;
    ranges = (function() {
      var _i, _len, _results;
      _results = [];
      for (_i = 0, _len = timelines.length; _i < _len; _i++) {
        t = timelines[_i];
        _results.push(getEventsDateRange(t.itemsData.get()));
      }
      return _results;
    })();
    if (mode === "date") {
      _results = [];
      for (i = _i = 0, _len = timelines.length; _i < _len; i = ++_i) {
      ...
      maxDuration = _.max((function() {
        ...
        for (_j = 0, _len1 = ranges.length; _j < _len1; _j++) {
          r = ranges[_j];
          _results1.push(r.max - r.min);
        }
        return _results1;
      })());
      _results1 = [];
      for (i = _j = 0, _len1 = timelines.length; _j < _len1; i = ++_j) {
        timeline = timelines[i];
        dateRange = ranges[i];
        dateRange = {
  ...
  renderEvents = function($container, events) {
    var bgEvent, event, items, options, timeline, _i, _len;
  ...
  deserializeEventRows = function(data) {
    ...
    for (sheetName in data) {
      ...
      if (_.all((function() {
        ...
        _ref = ["timestamp", "time", "action", "date"];
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          name = _ref[_i];
